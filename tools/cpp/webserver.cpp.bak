...
public:
    ...
    void bizmain(connection &conn,shared_ptr<st_recvmesg> &ptr)
    {
        ...
        getvalue(ptr->message,"username",username);    // 获取用户名。
        getvalue(ptr->message,"passwd",passwd);            // 获取密码。
        getvalue(ptr->message,"intername",intername);   // 获取接口名。
        ...
        if (stmt.next()!=0)
        {
            string message=sformat("HTTP/1.1 200 OK\r\n"
                                                    "Server: webserver\r\n"
                                                    "Content-Type: text/html;charset=utf-8\r\n");
            string data="<retcode>-1</retcode><message>用户名或密码不正确。</message>";
            message=message+sformat("Content-Length=%d\r\n\r\n",data.size())+data;
            ptr->message=message;
            return;
        }
        ...
        if (ip.empty()==false)
        {
            if (ip.find(clientmap[ptr->sock].clientip)==string::npos)
            {
                string message=sformat("HTTP/1.1 200 OK\r\n"
                                                        "Server: webserver\r\n"
                                                        "Content-Type: text/html;charset=utf-8\r\n");
                string data="<retcode>-1</retcode><message>用户的ip地址不正确。</message>";
                message=message+sformat("Content-Length=%d\r\n\r\n",data.size())+data;
                ptr->message=message;
                return;
            }
        ...
        if (icount==0)
        {
            string message=sformat("HTTP/1.1 200 OK\r\n"
                                                    "Server: webserver\r\n"
                                                    "Content-Type: text/html;charset=utf-8\r\n");
            string data="<retcode>-1</retcode><message>用户无权限，或接口不存在。</message>";
            message=message+sformat("Content-Length=%d\r\n\r\n",data.size())+data;
            ptr->message=message;
            return;
        }
        ...
        // 绑定查询数据的SQL语句的输入变量。
        // 根据接口配置中的参数列表（bindin字段），从URL中解析出参数的值，绑定到查询>数据的SQL语句中。
        ...
        {
            logfile.write("fieldname1=%s\n",cmdstr[ii].c_str());
            stmt.bindin1(ii+1,invalue[ii]);
            getvalue(ptr->message,cmdstr[ii],invalue[ii]);
            logfile.write("%s=%s\n",cmdstr[ii].c_str(),invalue[ii].c_str());
        }
        //////////////////////////////////////////////////
        ...
        string strsendbuffer; // 发送给客户端的xml。

        if (stmt.execute() != 0)
        {
            sformat(strsendbuffer,"<retcode>%d</retcode><message>%s</message>\n",stmt.rc(),stmt.message());
            logfile.write("stmt.execute() failed.\n%s\n%s\n",stmt.sql(),stmt.message()); return;
        }

        strsendbuffer="<retcode>0</retcode><message>ok</message>\n";

        // 向客户端发送xml内容的头部标签<data>。
        strsendbuffer=strsendbuffer+"<data>\n";
logfile.write("sql=%s\n",stmt.sql());
        ...
        while (true)
        {
            ...
            for (int ii=0;ii<cmdstr.size();ii++)
            {
                strsendbuffer=strsendbuffer+sformat("<%s>%s</%s>",cmdstr[ii].c_str(),colvalue[ii].c_str(),cmdstr[ii].c_str());
            }

            strsendbuffer=strsendbuffer+"<endl/>\n";   // xml每行结束的标志。
...
logfile.write("%s",strsendbuffer.c_str());
        string message=sformat("HTTP/1.1 200 OK\r\n"
                                                "Server: webserver\r\n"
                                                "Content-Type: text/html;charset=utf-8\r\n");
        message=message+sformat("Content-Length=%d\r\n\r\n",strsendbuffer.size())+strsendbuffer;
        ptr->message=message;
...
logfile.write("fieldnamea=%s\n",name.c_str());
    if (startp==string::npos) { logfile.write("=%s= not found\n",name.c_str()); 
//logfile.write("strget=%s\n",strget.c_str());
    return false; }
